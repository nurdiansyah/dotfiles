...
cmd_status() {
  ...
  if sudo launchctl print system/${DAEMON_LABEL} >/dev/null 2>&1; then
    sudo launchctl print system/${DAEMON_LABEL} | egrep 'pid =|state =|stdout path' || true
  else
    echo "daemon: not-loaded"
    sudo launchctl list | egrep 'kanata|org.nurdiansyah' || true
  fi
...
cmd_install_config() {
  ...
  sudo bash "$(pwd)/kanata/install-kanata-config.sh" --src "$SRC"
}
...
cmd_foreground() {
  ...
  if sudo launchctl print system/${DAEMON_LABEL} >/dev/null 2>&1; then
    info "Temporarily stopping system daemon (will not re-enable automatically)"
